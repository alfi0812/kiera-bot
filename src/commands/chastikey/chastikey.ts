...
export async function roleCounts(routed: RouterRouted) {
  ...
  routed.message.guild.roles.cache.forEach((r) => {
    ...
    if (r.name.toLowerCase() === 'lockee lvl 4') counts.devoted = counts.devoted + r.members.size
    if (r.name.toLowerCase() === 'lockee lvl 3') counts.experienced = counts.experienced + r.members.size
    if (r.name.toLowerCase() === 'lockee lvl 2') counts.intermediate = counts.intermediate + r.members.size
  ...
  response += `=========================================\n`
  response += `Keyholder Lvl 5          # ${counts.renowned} ${arrFrom(lgStr + 3 - strLen(counts.renowned)).join(' ')} ${rd(counts.renowned / cacheSize)}%\n`
  response += `Keyholder Lvl 4          # ${counts.distinguished} ${arrFrom(lgStr + 3 - strLen(counts.distinguished)).join(' ')} ${rd(counts.distinguished / cacheSize)}%\n`
  response += `Keyholder Lvl 3          # ${counts.established} ${arrFrom(lgStr + 3 - strLen(counts.established)).join(' ')} ${rd(counts.established / cacheSize)}%\n`
  response += `Keyholder Lvl 2          # ${counts.keyholder} ${arrFrom(lgStr + 3 - strLen(counts.keyholder)).join(' ')} ${rd(counts.keyholder / cacheSize)}%\n`
  response += `Keyholder Lvl 1          # ${counts.noviceKh} ${arrFrom(lgStr + 3 - strLen(counts.noviceKh)).join(' ')} ${rd(counts.noviceKh / cacheSize)}%\n`
  response += `=========================================\n`
  response += `Lockee Lvl 5             # ${counts.fanatical} ${arrFrom(lgStr + 3 - strLen(counts.fanatical)).join(' ')} ${rd(counts.fanatical / cacheSize)}%\n`
  response += `Lockee Lvl 4             # ${counts.devoted} ${arrFrom(lgStr + 3 - strLen(counts.devoted)).join(' ')} ${rd(counts.devoted / cacheSize)}%\n`
  response += `Lockee Lvl 3             # ${counts.experienced} ${arrFrom(lgStr + 3 - strLen(counts.experienced)).join(' ')} ${rd(counts.experienced / cacheSize)}%\n`
  response += `Lockee Lvl 2             # ${counts.intermediate} ${arrFrom(lgStr + 3 - strLen(counts.intermediate)).join(' ')} ${rd(counts.intermediate / cacheSize)}%\n`
  response += `Lockee Lvl 1             # ${counts.noviceLockee} ${arrFrom(lgStr + 3 - strLen(counts.noviceLockee)).join(' ')} ${rd(counts.noviceLockee / cacheSize)}%\n`
  response += `\`\`\``
